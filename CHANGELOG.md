...
- **Repo is TestFlight-ready for iOS, tvOS, and visionOS.** Three pieces landed:
  - `ci_scripts/ci_post_clone.sh` — Xcode Cloud installs XcodeGen via Homebrew
    and runs `xcodegen generate` inside the cloned workspace before
    `xcodebuild` ever fires. Aether's `.xcodeproj` stays out of git.
  - `ci_scripts/ci_pre_xcodebuild.sh` — patches `CFBundleVersion` on every
    platform's `Info.plist` to `$CI_BUILD_NUMBER` (PlistBuddy), so each cloud
  ...
  Plus `ITSAppUsesNonExemptEncryption: false` on all three Info.plists so
  App Store Connect skips the encryption export compliance question every
  build (Aether ships only Apple URLSession HTTPS).

### Documentation

- Refreshed the 0.5 distribution plan to reflect the above: the repo-side
  gaps are closed; what remains is the one-time Apple-side setup (developer
  account, registered bundle IDs, three App Store Connect app records, the
  `Aether Internal` tester group, and three Xcode Cloud workflows — one per
  platform). Step-by-step checklist now in
  [`docs/next-steps/0.5-distribution.md`](docs/next-steps/0.5-distribution.md).
- Original 0.5 distribution plan for internal TestFlight using Xcode Cloud:
